...
2. **Filter-apply readiness wait** (Task #59 → superseded by Task #62, see below). The original 2-second inline wait inside `applyFilterBySelectIndex` set the closure-scoped `_scheduleNotLoaded` flag and bailed with the new distinct reason `schedule_not_loaded` (instead of `filter_apply_failed`) — skipping the noisy unfiltered scan. The unfiltered-fallback skip is preserved; only the wait location and budget changed in Task #62.

**Apr 17 — schedule readiness wait centralized (Task #62).** Job #10 Gentle Yoga Tuesday and Job #24 Flow Yoga Friday were repeatedly bailing with `schedule_not_loaded` because the inline 2-second wait inside `applyFilterBySelectIndex` was too aggressive for slow Bubble.io hydration at booking-window open time. The wait was moved out of the filter helper and now runs ONCE just after navigation (`register-pilates.js` ~1044-1115):
- **10 s budget** (vs. old 2 s) with a 200 ms poll interval.
- **Multi-signal probe**: exits early on (a) `"X classes this week"` count text > 0, OR (b) a visible leaf element matching the time-of-day pattern (`\d{1,2}:\d{2}\s*(AM|PM)`), OR (c) `spinner_gone` — but only when a spinner was previously observed during the wait AND the count widget has rendered (`count !== null`), to avoid mis-classifying an empty-from-the-start schedule as ready. The fired signal is logged (`signal=count_text=N`, `signal=card_visible`, or `signal=spinner_gone(count=N)`).
- **Per-probe progress logs (~1 Hz)** while waiting, showing elapsed ms, poll count, and current probe values so slow renders are visible in run logs.
- **One in-run reload retry**: if the initial 10 s wait fails, performs exactly one `page.reload()` + dropdown re-wait + readiness re-probe. Only after the reload also fails does it set `_scheduleNotLoaded = true` so the existing Task #59 bail path fires.
- The inline 2 s wait inside `applyFilterBySelectIndex` was deleted; the helper now short-circuits with `_scheduleNotLoaded ? return false` so the caller's bail path stays the single source of truth. The unfiltered-fallback skip from Task #59 is preserved unchanged.
