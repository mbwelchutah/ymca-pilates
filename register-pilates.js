...
(async () => {
   ...
   // Step 5: Click Wednesday's date tab
   ...
   // Step 7: Try to register — retry up to 10 minutes if registration not open yet
   ...
   for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      if (hasRegister) {
              ...
              console.log(`Successfully registered on attempt ${attempt}!`);
              registered = true;
              break;
      } else if (hasWaitlist) {
              await waitlistBtn.first().click();
              console.log(`Class full — joined waitlist on attempt ${attempt}.`);
              registered = true;
              break;
      } else {
              console.log(`Attempt ${attempt}/20: Registration not open yet. Waiting 30s...`);
              await page.waitForTimeout(30000);
              await page.reload();
              await page.waitForLoadState('networkidle');
              await page.waitForTimeout(3000);
              // Re-click Wednesday tab and class card after reload
   ...
   if (!registered) {
         console.log('Registration did not open within 10 minutes. Giving up.');
         process.exit(1);
   }
